...
export function ServiceEditPage({ token, categories, defaultUniversity }: ServiceEditPageProps) {
  ...
  useEffect(() => {
    ...
    if (!serviceId || Number.isNaN(serviceId)) {
      setError(t("service_edit.error.invalid_id"));
      return;
    }
    let isActive = true;
    setIsLoading(true);
  ...
  if (!token) {
    return (
      <section className="section-grid">
        <p className="error-box">{t("service_edit.auth_warning")}</p>
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        ...
        setError(t("service_edit.error.update_failed"));
      }
    } finally {
  ...
  if (isLoading || !service) {
    return (
      <section className="section-grid">
        {error ? <p className="error-box">{error}</p> : <p className="muted">{t("service_edit.loading")}</p>}
      </section>
    );
  }

  const displayPreviews = images.length > 0 ? previews : service.images;
  const canRemove = images.length > 0;

  return (
    <section className="section-grid service-publish">
      <div>
        <p className="eyebrow">{t("service_edit.eyebrow")}</p>
        <h1>{t("service_edit.title")}</h1>
      </div>
      ...
      <form className="form-card" onSubmit={handleSubmit}>
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              alignItems: "center",
              justifyContent: displayPreviews.length > 0 ? "flex-start" : "center",
              gap: "12px",
              width: "100%",
              maxWidth: "100%",
              boxSizing: "border-box",
            ...
            {displayPreviews.length > 0 && (
              ...
              >
                {displayPreviews.map((preview, index) => (
                  <div key={`${preview}-${index}`} style={{ flex: "0 0 auto", scrollSnapAlign: "start", position: "relative" }}>
                    <img
                      src={preview}
                      alt={`preview-${index + 1}`}
                      style={{
        ...
        <button className="primary" disabled={isSubmitting}>
          {isSubmitting ? t("service_edit.form.submitting") : t("service_edit.form.submit")}
        </button>
      </form>
    </section>
  );
